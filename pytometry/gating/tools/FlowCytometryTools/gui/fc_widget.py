...
from gating.tools.FlowCytometryTools.core.utils import to_list
...
class FCGateManager(EventGenerator):
    ...
    def __init__(self, ax, callback_list=None):
        ...
        #names blacklist Felix
        self.names_blacklist = list()
    ...
    def _get_next_gate_name(self, taken_flag=False):
        message = "Insert gate name"
        if taken_flag:
            message = "A gate with this name already exists. Please insert another name"
        gate_name = self._get_name_from_dialog(message=message)#'gate{0}'.format(self.gate_num) #Felix
        if gate_name is '' or gate_name is None:
            gate_name = 'gate%d' %numpy.random.randint(0,9999)
            while gate_name in self.names_blacklist:
                gate_name = 'gate%d' %numpy.random.randint(0,9999)
        if gate_name in self.names_blacklist:
            return self._get_next_gate_name(taken_flag=True)
        self.names_blacklist.append(gate_name)
        return gate_name

    def rem_name_from_gate_blacklist(self, name):
        if name in self.names_blacklist:
            self.names_blacklist.remove(name)

    def _get_name_from_dialog(self, parent=None, message='', default_value=''):
        import wx
    ...
    def _get_region_from_dialog(self, kind):
        import wx
        choices = []
        if kind == 'poly':
            choices = ['in', 'out']
        elif 'thresh' in str(kind):
            choices = ['above', 'below']
        elif kind == 'quad':
    ...
    def create_gate_widget(self, kind):
        ...
        def create_gate(*args):
            ...
            if not cancelled:
                ...
                gate.region = self._get_region_from_dialog(kind)
                gate.spawn(ch, self.ax)
                self.add_gate(gate)#kind
    ...
    def get_generation_code(self):
        ...
        if len(self.gates) < 1:
            ...
            import_list = 'from anndata_tools.Tools.FlowCytometryTools import ' + ', '.join(import_list)
    ...
    def get_gates(self):  #Felix
        ...
        for g in base_gates:
            ...
            name = g.name
            region=g.region
            if g.spawn_list[0].gate_type == 'ThresholdGate':
                transf_gates.add(fctgates.ThresholdGate(verts[0][0], channels, region=region, name=name))
            if g.spawn_list[0].gate_type == 'PolyGate':
                transf_gates.add(fctgates.PolyGate(verts, channels, region=region, name=name))
            if g.spawn_list[0].gate_type == 'QuadGate':
                transf_gates.add(fctgates.QuadGate(verts[0][0], channels, region=region, name=name))
        return transf_gates


    def get_base_gates(self):
        return set([gate for gate in self.gates])
